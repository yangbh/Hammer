...
def usage():
 ...
 print "       -b: Data source (google,baidu,bing,bingapi,pgp,linkedin,google-profiles,people123,jigsaw,baidu-google,all)"
 ...
 print "         ./theharvester.py -d microsoft.com -l 500 -b baidu"
 print "         ./theharvester.py -d microsoft.com -b baidu-google -l 500 "
...
def start(argv):
	...
	for opt, arg in opts:
		if opt == '-l' :
			...
			engine = arg
			if engine not in ("google", "baidu", "linkedin", "pgp", "all","google-profiles","bing","bing_api","yandex","people123","jigsaw","baidu-google"):
				usage()
				print "Invalid search engine, try with: baidu,bing, google, linkedin, pgp, exalead, jigsaw, bing_api, people123, google-profiles,baidu-google"
				sys.exit()
			else:
				pass
	if engine == "google":
	...
	if engine == "baidu":
		...
		all_emails=search.get_emails()
		all_hosts=search.get_hostnames()
		#print word,limit,start,all_hosts
	if engine == "exalead":
		...
		for users in people:
			print users
		sys.exit()
	elif engine == "baidu-google":
		print "Baidu and google harvest.."
		all_emails=[]
		all_hosts=[]
		virtual = "basic"
		print "[-] Searching in Google.."
	...
	if all_emails ==[]:
		...
		xemail = list(set(all_emails))#add by ha.cker list(set) is use for Remove duplicate email
		#for emails in all_emails:
		for emails in xemail:
			print emails 
	...
	if all_hosts == []:
		...
		full_host=hostchecker.Checker(xhost)
